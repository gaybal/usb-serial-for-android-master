...
import com.hoho.android.usbserial.examples.MainApplication;
 ...
 */
public class SerialConsoleActivityI extends Activity implements AdapterView.OnItemSelectedListener{

    private final String TAG = SerialConsoleActivityI.class.getSimpleName();

    private Context mContext = SerialConsoleActivityI.this;
    ...
    private final SerialInputOutputManager.Listener mListener =
        ...
        public void onNewData(final byte[] data) {
            SerialConsoleActivityI.this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    SerialConsoleActivityI.this.updateReceivedData(data);
                }
            });
        }
    };
...
//        USBManager.getInstance(mContext).setOnInputListener(mListener);
    ...
    public void sendCmdToPlc(View v){
        USBDevice device = ((MainApplication)getApplication()).mEntries.get(0);
        byte[] ret = ModbusTransfer.getInstance(mContext).sendCmdToPlc(device,
...
//        Toast.makeText(mContext, StringUtil.byte2HexStr(ret), Toast.LENGTH_SHORT).show();
    ...
    public void readDataFromPlc(View v){
        new Thread(new Runnable() {
            ...
            public void run() {
                USBDevice device = ((MainApplication)getApplication()).mEntries.get(0);
                final byte[] ret = ModbusTransfer.getInstance(mContext).readDataFromPlc(device,
